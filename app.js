...
const renderTheme = (themes) => {
    ...
    for (let i = 0; i < themes.length; i += 1) {
        ...
        p.innerHTML = `
            <span class="selection__result-ration"> ${result}/${themes[i].list.length} </span>
...
const saveResult = (result, id) => localStorage.setItem(id, result);
...
const showResult = (result, quiz) => {
    ...
    button.textContent = 'списику квизов';
